...
def svg_16() -> str:
    ...
    parts = [
        ...
        label(1160, 290, "• Global prefix KV cache", C["ink"], 12, "start"),
        label(1160, 315, "• One god ServiceAccount", C["ink"], 12, "start"),
        label(1160, 340, "• Public engine port", C["ink"], 12, "start"),
        label(1160, 365, "• Open pod egress", C["ink"], 12, "start"),
...
def svg_17() -> str:
    ...
    parts = [
        ...
        title_bar(W, "Example C — Secure self-hosted serving platform",
                  "Signed artifacts at admission · tenant-bound KV · GPU tiers · gateway in front of the engine · CT same path.",
                  "SECURED", C["ok"]),
        ...
        label(560, 118, "Engine key ≠ data-plane PEP", C["muted"], 12, "start"),
        card(36, 150, 200, 56, "1", "Calling apps", "still own RAG / OBO", "#EEF2FF", "#6366F1", "#6366F1", p),
        gate(260, 152, 230, 52, "G", "AI Gateway", "engine authN · quota · audit", p),
        ...
        label(56, 258, "ADMIT PATH (artifacts)", C["control_hd"], 13, "start", "800"),
        gate(56, 275, 460, 48, "AD", "Admission", "cosign · digest pin · ModelScan", p),
        card(56, 340, 220, 56, "4", "Registry", "signed OCI / snapshots", C["panel"], C["control_bd"], C["control_bd"], p),
        card(296, 340, 220, 56, "8", "Adapters", "same Path 1 as weights", C["panel"], C["control_bd"], C["control_bd"], p),
        card(56, 420, 460, 70, "11", "CT / LoRA promote", "scan · sign · admit · Evidence Pack — no side door", C["panel"], C["control_bd"], C["control_bd"], p),
        f'<rect x="556" y="230" width="500" height="300" rx="14" fill="{C["control_bg"]}" stroke="{C["control_bd"]}" stroke-width="1.4" filter="url(#{p}soft)"/>',
        label(576, 258, "SERVE PATH (inference)", C["control_hd"], 13, "start", "800"),
        gate(576, 275, 460, 48, "KV", "Tenant KV bind", "key = (tenant, session) · cleanup", p),
        card(576, 340, 220, 56, "5", "vLLM / KServe", "NetworkPolicy to engine", C["panel"], C["control_bd"], C["control_bd"], p),
        card(816, 340, 220, 56, "7", "KV / CAG", "lifecycle: create · purge", C["panel"], C["control_bd"], C["control_bd"], p),
        card(576, 420, 460, 70, "6", "GPU pool", "MIG or dedicated nodes for sensitive tiers · sanitize pages", C["panel"], C["control_bd"], C["control_bd"], p),
        f'<rect x="1076" y="230" width="468" height="180" rx="14" fill="{C["action_bg"]}" stroke="{C["action_bd"]}" filter="url(#{p}soft)"/>',
        label(1096, 258, "CLUSTER (Plane C)", C["action_bd"], 12, "start", "800"),
        card(1096, 275, 428, 56, "9", "Serving identity", "engine key authenticates TO the GPU", C["panel"], C["action_bd"], C["action_bd"], p),
        card(1096, 345, 428, 48, "10", "Default-deny + egress allowlist", "no public :8000", C["panel"], C["action_bd"], C["action_bd"], p),
        f'<rect x="1076" y="430" width="468" height="100" rx="14" fill="{C["model_bg"]}" stroke="{C["model_bd"]}" filter="url(#{p}soft)"/>',
        label(1096, 460, "Prefix cache = security setting", C["model_hd"], 12, "start", "800"),
        label(1096, 490, "OFF or tenant-scoped on sensitive tiers — never global sticky prefix.", C["ink"], 11, "start"),
        f'<rect x="36" y="560" width="1508" height="100" rx="12" fill="{C["panel"]}" stroke="{C["ok"]}" stroke-width="1.5"/>',
        label(56, 590, "Primary enforcement chain", C["ok"], 14, "start", "800"),
        label(56, 618, "signed artifact → admit to GPU tier → gateway engine authN → KV key bind → generate → session cleanup", C["ink"], 13, "start"),
        label(56, 642, "A correct engine key does not make Bob's chunks legal in Alice's prompt. Application ACL stays in Plane A (B/D).", C["muted"], 12, "start"),
        f'<rect x="36" y="680" width="740" height="260" rx="12" fill="{C["panel"]}" stroke="{C["line"]}"/>',
        label(56, 710, "Two identities", C["ink"], 14, "start", "800"),
        label(56, 745, "Engine key / SA: may this caller hit this replica?", C["ink"], 12, "start"),
        label(56, 775, "App user token: whose data is in the prompt? (not here)", C["ink"], 12, "start"),
        label(56, 815, "Anti-pattern: one cluster-wide ServiceAccount", C["danger"], 12, "start"),
        label(56, 845, "Anti-pattern: prefix cache ON for multi-tenant GPU", C["danger"], 12, "start"),
        label(56, 875, "Anti-pattern: LoRA hot-load from an unsigned blob", C["danger"], 12, "start"),
        f'<rect x="800" y="680" width="744" height="260" rx="12" fill="{C["panel"]}" stroke="{C["line"]}"/>',
        label(820, 710, "Control map", C["ink"], 14, "start", "800"),
        gate(820, 735, 340, 48, "AD", "Admit", "cosign · scan · digest", p),
        gate(1180, 735, 340, 48, "KV", "KV bind", "tenant · session · purge", p),
        gate(820, 805, 340, 48, "G", "Gateway", "no public engine", p),
        gate(1180, 805, 340, 48, "GPU", "Isolate", "MIG / sanitize / deny egress", p),
        label(820, 890, "YAML stays in Chapter 16 — this card is the composition.", C["muted"], 12, "start"),
        "</svg>",
    ]
    return "\n".join(parts)
...
"""
    ...
    parts = [
        ...
        title_bar(W, "Example C — Admit + serve data flows",
                  "Left: artifact admit (write). Right: generate (read). CT and LoRA re-enter the left path. KV keys bind tenant + session.",
                  "DATA FLOW", C["accent"]),
        ...
        step(210, "A2", "ModelScan + signature", "cosign verify against trusted keys", "control"),
        step(270, "A3", "Admission policy", "digest pin · namespace = GPU tier", "control"),
        step(330, "A4", "Schedule engine", "never :latest · NetworkPolicy to port", "action"),
        ...
        step_r(150, "S1", "Authenticate to engine", "caller mTLS / engine key — not data ACL", "control"),
        step_r(210, "S2", "Attest tenant", "from calling app identity — not prompt JSON", "control"),
        step_r(270, "S3", "Bind KV key", "(tenant, session) · prefix cache off or scoped", "control"),
        step_r(330, "S4", "Generate", "engine.generate(prompt, kv_partition=key)", "model"),
        step_r(390, "S5", "Cleanup", "purge KV · sanitize GPU pages on session end", "control"),
        f'<rect x="36" y="470" width="700" height="200" rx="12" fill="{C["panel"]}" stroke="{C["ok"]}"/>',
        label(56, 500, "CT / LoRA re-enter Path 1", C["ok"], 14, "start", "800"),
        label(56, 530, "There is no side door that says \"it's only an adapter.\"", C["ink"], 12, "start"),
        label(56, 555, "Engine authentication is not application authorization.", C["ink"], 12, "start"),
        label(56, 580, "YAML and Helm live in Chapter 16; this is the composition.", C["ink"], 12, "start"),
        label(56, 620, "Managed API instead? Skip C; use E.1.2 + Appendix D.", C["muted"], 12, "start"),
        f'<rect x="760" y="470" width="680" height="200" rx="12" fill="{C["panel"]}" stroke="{C["danger"]}"/>',
        ...
        label(780, 560, "• PromptPeek-class shared prefix cache", C["ink"], 12, "start"),
        label(780, 585, "• LoRA hot-load skipping ModelScan", C["ink"], 12, "start"),
        label(780, 610, "• Leftover KV after session end", C["ink"], 12, "start"),
        label(780, 635, "• Anonymous generate on public :8000", C["ink"], 12, "start"),
        f'<rect x="36" y="700" width="1404" height="100" rx="12" fill="{C["panel"]}" stroke="{C["line"]}"/>',
        label(56, 735, "Honest residual", C["ink"], 13, "start", "800"),
        label(56, 760, "Host/CSP with keys can still read externalized KV (obfuscation is Emerging).", C["muted"], 12, "start"),
        label(56, 785, "A perfectly isolated GPU still answers from Bob's chunk if the app put it in the prompt — see Example B/D.", C["muted"], 12, "start"),
        f'<rect x="36" y="830" width="1404" height="220" rx="12" fill="{C["control_bg"]}" stroke="{C["control_bd"]}"/>',
        label(56, 860, "Pseudo-code (serve)", C["control_hd"], 13, "start", "800"),
        label(56, 890, "app = verify_engine_caller(req)          # not data ACL", C["ink"], 12, "start"),
        label(56, 915, "kv_key = bind(tenant=app.tenant, session=req.session)", C["ink"], 12, "start"),
        label(56, 940, "out = engine.generate(req.prompt, kv_partition=kv_key)", C["ink"], 12, "start"),
        label(56, 965, "cleanup_kv(kv_key); sanitize_gpu_pages()", C["ink"], 12, "start"),
        label(56, 1000, "Prefix cache: OFF or tenant-scoped on sensitive tiers.", C["muted"], 12, "start"),
        "</svg>",
    ]
    return "\n".join(parts)
